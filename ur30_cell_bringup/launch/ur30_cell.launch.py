...
def launch_setup(context, *args, **kwargs):
    ...
    # The vendored orbbec_description meshes are referenced via package://
    # URIs, which sdformat rewrites to model:// -- point Gazebo's resource
    # search path at the package's install share dir so those resolve.
    orbbec_resource_path = AppendEnvironmentVariable(
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "ur_type",
            description="Type/series of used UR robot. This cell's pedestal/table/door-panel "
            "geometry was sized around the UR30's ~1.3m reach -- re-check reach if changed.",
            choices=[
                "ur3",
                "ur5",
                "ur10",
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "description_file",
            default_value=PathJoinSubstitution(
                [FindPackageShare("ur30_cell_description"), "urdf", "ur30_cell.xacro"]
            ),
            description="URDF/XACRO description file (absolute path) with the workcell.",
        )
    )
    declared_arguments.append(
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "rviz_config_file",
            default_value=PathJoinSubstitution(
                [FindPackageShare("ur30_cell_bringup"), "rviz", "ur30_cell.rviz"]
            ),
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "world_file",
            default_value=PathJoinSubstitution(
                [FindPackageShare("ur30_cell_bringup"), "worlds", "ur30_cell.world.sdf"]
            ),
            description="Gazebo world file (absolute path or filename from the gazebosim worlds collection) containing the workcell world.",
        )
    )
    declared_arguments.append(
        DeclareLaunchArgument(
